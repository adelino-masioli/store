@if(isset($payment))
    ...
    <a href="{{route('order-edit', [$order->id])}}" class="btn btn-sm bg-yellow btn-flat"><i class="fa fa-arrow-left" aria-hidden="true"></i> Voltar</a>
@else
    ...
    @if(isset($items) && $items->count() > 0)
        <a href="{{route('order-next-payment', [$order->id])}}" class="btn btn-sm bg-yellow btn-flat">Avançar <i class="fa fa-arrow-right" aria-hidden="true"></i></a>
    @else
...
@if(isset($payment))
    <a href="{{route('order-next-payment-confirm', [$order->id])}}" class="btn btn-sm bg-green btn-flat">Confirmar Pagamento e finalizar <i class="fa fa-dollar" aria-hidden="true"></i></a>
@else
