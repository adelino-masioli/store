                ...
                <div class="col-md-12">
                    <table class="table table-bordered table-condensed table-hover table-striped" style="width: 100%;">
                        ...
                        @foreach($documents->take(3) as $document)
                            <tr>
                                <td class="text-center"><a href="{{route('document-edit', [$document->id])}}" class="btn btn-flat btn-xs bg-info"><i class="fa fa-search"></i></a></td>
                                <td>{{$document->name}}</td>
                                <td>{{\App\Models\Document::user($document->id)}}</td>
                                <td>{{$document->type->type}}</td>
                                <td class="text-center">
                                    @if($document->file)
                                        <a href="{{route('document-download', $document->file)}}"  title="Baixar" class="btn bg-green btn-xs"><i class="fa fa-download"></i></a>
                                    @else
