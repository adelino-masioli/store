...
trait DataTableTrait
{
    ...
    {
        if($nostatus==null){
            $result = $model->select($columns)->where('configuration_id', Auth::user()->configuration_id)->where('type', $type)->where('status_id', '!=', statusOrder('canceled'))->orderBy('id', 'desc');
        }else{
            $result = $model->select($columns)->where('configuration_id', Auth::user()->configuration_id)->where('type', $type)->orderBy('id', 'desc');
        }
        return $result;
    }
}